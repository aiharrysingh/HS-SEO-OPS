...
export function ImportPagesButton({ clientId }: { clientId: string }) {
  ...
  const run = async () => {
    ...
    setState({ kind: "idle" });
    try {
      const res = await fetch(`/api/clients/${clientId}/import-pages`, {
        method: "POST",
      });
      const body = await res.json();

      if (!res.ok) {
        setState({ kind: "error", message: body.error ?? "Import failed" });
        return;
      }
      ...
      setState({
        ...
        message:
          body.created > 0
            ? `Added ${body.created} page${body.created === 1 ? "" : "s"}. Set each one's publish date and target keyword to unlock the milestone columns.`
            : "No new pages — everything Search Console reports is already tracked.",
      });
